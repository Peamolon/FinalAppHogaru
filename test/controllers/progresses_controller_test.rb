...
class ProgressesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create progress" do
    assert_difference('Progress.count') do
      post progresses_url, params: { progress: { burnedCalories: @progress.burnedCalories, consumedCalories: @progress.consumedCalories, expires_at: @progress.expires_at, porcent: @progress.porcent, user_id: @progress.user_id } }
    end
  ...
  test "should update progress" do
    patch progress_url(@progress), params: { progress: { burnedCalories: @progress.burnedCalories, consumedCalories: @progress.consumedCalories, expires_at: @progress.expires_at, porcent: @progress.porcent, user_id: @progress.user_id } }
